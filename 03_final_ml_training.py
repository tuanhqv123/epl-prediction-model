#!/usr/bin/env python3
"""
FINAL ML TRAINING WITH PROPERLY ENGINEERED FEATURES
Train models on the corrected dataset with proper home/away momentum features
"""
...
def load_datasets():
    """Load baseline and properly enhanced datasets"""
    ...
    # Load fixed enhanced data
    try:
        enhanced_df = pd.read_csv('data_processed/fixed_enhanced_epl_ml.csv')
        print(f"   ✅ Fixed Enhanced: {len(enhanced_df)} matches")
    except FileNotFoundError:
        print("   ❌ Fixed enhanced data not found")
...
def split_data_temporal(X, y, test_size=0.2):
    ...
    print(f"         📅 Test range: {min(test_indices)}-{max(test_indices)}")
...
def train_advanced_models(X_train, y_train, X_test, y_test, dataset_name):
    """Train advanced models with proper evaluation"""
    print(f"   🤖 Training advanced models on {dataset_name}...")
...
def analyze_results(baseline_results, enhanced_results):
    """Analyze and compare baseline vs enhanced results"""
...
def identify_best_model(results, dataset_name):
    ...
    for name, model_results in results.items():
        # Consider both test accuracy and CV score for robustness
        combined_score = (model_results['test_accuracy'] + model_results['cv_mean']) / 2
    ...
    # Check draw prediction performance
    draw_report = best_model['classification_report'].get('D', {})
    draw_precision = draw_report.get('precision', 0)
    draw_recall = draw_report.get('recall', 0)
    draw_f1 = draw_report.get('f1-score', 0)

    print(f"\n🎯 Draw Prediction Performance:")
    print(f"   📊 Precision: {draw_precision:.1%}")
    print(f"   📊 Recall: {draw_recall:.1%}")
    print(f"   📊 F1-Score: {draw_f1:.1%}")

    return best_model, best_name

def create_final_visualizations(baseline_results, enhanced_results, comparison_df):
    ...
    plt.axhline(y=0.05, color='orange', linestyle='--', alpha=0.5, label='Acceptable Gap')
    ...
    # 5. Model Rankings
    plt.subplot(2, 3, 5)
    model_rankings = sorted([(name, baseline_results[name]['test_accuracy'])
                           for name in models], key=lambda x: x[1], reverse=True)

    ranking_names = [name[0].replace('_', ' ').title() for name in model_rankings]
    ranking_scores = [name[1] for name in model_rankings]

    plt.barh(range(len(ranking_names)), ranking_scores, color='steelblue', alpha=0.7)
    ...
    Best Enhanced: {best_enhanced:.1%}
    Avg Improvement: {avg_improvement:+.1%}

    Low Overfitting: {sum(1 for gap in overfitting_gaps if gap < 0.05)}/{len(models)} models
    High Overfitting: {sum(1 for gap in overfitting_gaps if gap > 0.1)}/{len(models)} models

    Stable CV (σ < 0.02): {sum(1 for std in cv_stds if std < 0.02)}/{len(models)} models
    """
    ...
    plt.tight_layout()
    plt.savefig('final_ml_results/comprehensive_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def save_final_results(baseline_results, enhanced_results, comparison_df, best_analysis):
    ...
    final_results = {
        ...
        'best_baseline_model': {
            'name': best_analysis[0][1],
            'accuracy': best_analysis[0][0]['test_accuracy'],
            'cv_score': best_analysis[0][0]['cv_mean'],
            'overfitting_gap': best_analysis[0][0]['overfitting_gap']
        }
    }

    with open(f'{results_dir}/final_ml_results.pkl', 'wb') as f:
    ...
    # Save best model
    best_model = best_analysis[0][0]['model']
...
def main():
    ...
    print("🚀 FINAL ML TRAINING WITH PROPER FEATURES")
    print("=" * 60)
    ...
    print("\n🤖 TRAINING MODELS")
    print("-" * 20)
    baseline_results = train_advanced_models(X_base_train, y_base_train, X_base_test, y_base_test, "Baseline")
    enhanced_results = train_advanced_models(X_enh_train, y_enh_train, X_enh_test, y_enh_test, "Enhanced") if enhanced_df is not None else None

    # Analyze results
    comparison_df = analyze_results(baseline_results, enhanced_results)

    # Identify best models
    best_baseline = identify_best_model(baseline_results, "Baseline")
    best_enhanced = identify_best_model(enhanced_results, "Enhanced") if enhanced_results else None

    # Create visualizations
    create_final_visualizations(baseline_results, enhanced_results, comparison_df)
    ...
    print(f"\n🏆 FINAL RESULTS:")
    print(f"   🥇 Best Baseline: {best_baseline[1].replace('_', ' ').title()} - {best_baseline[0]['test_accuracy']:.1%}")
    if enhanced_results:
        print(f"   🚀 Best Enhanced: {best_enhanced[1].replace('_', ' ').title()} - {best_enhanced[0]['test_accuracy']:.1%}")
