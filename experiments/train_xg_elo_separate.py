...
# New: K factor for xG-based ELO
K_XG = 20

print("SEPARATE XG-BASED ELO")
print("-" * 60)
...
def create_features_with_xg_elo(df, defaults, k_xg):
    # Regular ELO (based on results)
    elo = {}
    # Attack/Defense ELO (based on goals)
    elo_att, elo_def = {}, {}
    # NEW: xG-based ELO (based on xG performance)
    elo_xg = {}
    ...
    for _, m in df.iterrows():
        ...
        features.append({
            ...
            "h_xg_elo": h_xg_elo, "a_xg_elo": a_xg_elo,
        })
        ...
        # Update att/def ELO (based on goals)
        ...
        # Update xG ELO (based on xG)
        match_xg = get_xg(home, away, match_date.date())
        if match_xg:
            h_xg = match_xg['h_xG']
            a_xg = match_xg['a_xG']
            # xG-based result: who "should have" won based on xG
...
FEATURE_COLS_XG = FEATURE_COLS_BASE + ["xg_elo_diff"]
...
for k_xg in [0, 5, 10, 15, 20, 25, 30, 40, 50]:
    feat_df = create_features_with_xg_elo(df, defaults, k_xg)
    if k_xg == 0:
        loss, acc, _ = train_and_evaluate(feat_df, FEATURE_COLS_BASE, f"K_XG={k_xg} (baseline)")
    else:
        loss, acc, _ = train_and_evaluate(feat_df, FEATURE_COLS_XG, f"K_XG={k_xg}")
    if loss < best_loss:
        best_loss = loss
        best_k = k_xg

print(f"\nBest: K_XG={best_k}, Loss={best_loss:.4f}")
