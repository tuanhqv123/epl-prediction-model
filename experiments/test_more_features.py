...
print("CALCULATING ADDITIONAL SHOT METRICS...")

shot_metrics = shots.groupby('match_id').apply(
    lambda x: pd.Series({
        'h_counter_xg': x[(x['h_a'] == 'h') & (x['situation'].isin(['FromCorner', 'SetPiece']))]['xG'].sum(),
        'a_counter_xg': x[(x['h_a'] == 'a') & (x['situation'].isin(['FromCorner', 'SetPiece']))]['xG'].sum(),
        'h_open_play_xg': x[(x['h_a'] == 'h') & (x['situation'] == 'OpenPlay')]['xG'].sum(),
        'a_open_play_xg': x[(x['h_a'] == 'a') & (x['situation'] == 'OpenPlay')]['xG'].sum(),
        'h_header_xg': x[(x['h_a'] == 'h') & (x['shotType'] == 'Head')]['xG'].sum(),
        'a_header_xg': x[(x['h_a'] == 'a') & (x['shotType'] == 'Head')]['xG'].sum(),
        'h_penalty_xg': x[(x['h_a'] == 'h') & (x['situation'] == 'Penalty')]['xG'].sum(),
        'a_penalty_xg': x[(x['h_a'] == 'a') & (x['situation'] == 'Penalty')]['xG'].sum(),
    })
).reset_index()
xg_df = xg_df.merge(shot_metrics, on='match_id')
...
for _, row in xg_df.iterrows():
    ...
    data = {
        ...
        'h_counter_xg': row['h_counter_xg'], 'a_counter_xg': row['a_counter_xg'],
        'h_open_play_xg': row['h_open_play_xg'], 'a_open_play_xg': row['a_open_play_xg'],
        'h_header_xg': row['h_header_xg'], 'a_header_xg': row['a_header_xg'],
        'h_penalty_xg': row['h_penalty_xg'], 'a_penalty_xg': row['a_penalty_xg'],
    }
...
for ref in referee_stats:
    referee_stats[ref]['home_rate'] = referee_stats[ref]['home_wins'] / referee_stats[ref]['total'] if referee_stats[ref]['total'] >= 10 else 0.446
...
def calculate_defaults(df_subset):
    ...
    defaults['avg_set_piece_xg'] = 0.3
    defaults['avg_open_play_xg'] = 1.0
    defaults['avg_header_xg'] = 0.15
    return defaults
...
def create_features_all(df, defaults, new_features=None):
    ...
    for _, m in df.iterrows():
        ...
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [],
                                        "yellows": [], "corners": [], "set_piece_xg": [], "open_play_xg": [], "header_xg": []})
        ...
        h_set_piece = np.mean(h_hist["set_piece_xg"][-5:]) if h_hist["set_piece_xg"] else d['avg_set_piece_xg']
        a_set_piece = np.mean(a_hist["set_piece_xg"][-5:]) if a_hist["set_piece_xg"] else d['avg_set_piece_xg']
        h_open_play = np.mean(h_hist["open_play_xg"][-5:]) if h_hist["open_play_xg"] else d['avg_open_play_xg']
        a_open_play = np.mean(a_hist["open_play_xg"][-5:]) if a_hist["open_play_xg"] else d['avg_open_play_xg']
        h_header = np.mean(h_hist["header_xg"][-5:]) if h_hist["header_xg"] else d['avg_header_xg']
        a_header = np.mean(a_hist["header_xg"][-5:]) if a_hist["header_xg"] else d['avg_header_xg']
        ...
        if 'yellows_diff' in new_features: feat['yellows_diff'] = h_yellows - a_yellows
        if 'corners_diff' in new_features: feat['corners_diff'] = h_corners - a_corners
        if 'set_piece_xg_diff' in new_features: feat['set_piece_xg_diff'] = h_set_piece - a_set_piece
        if 'open_play_xg_diff' in new_features: feat['open_play_xg_diff'] = h_open_play - a_open_play
        if 'header_xg_diff' in new_features: feat['header_xg_diff'] = h_header - a_header
        if 'goal_diff_momentum' in new_features:
            h_gd = sum([1 if r == 'W' else -1 for r in h_hist["results"][-3:]]) if h_hist["results"] else 0
            a_gd = sum([1 if r == 'W' else -1 for r in a_hist["results"][-3:]]) if a_hist["results"] else 0
            feat['goal_diff_momentum'] = h_gd - a_gd
        ...
        for team, is_home in [(home, True), (away, False)]:
            ...
            if match_xg:
                team_stats[team]["set_piece_xg"].append(match_xg['h_counter_xg'] if is_home else match_xg['a_counter_xg'])
                team_stats[team]["open_play_xg"].append(match_xg['h_open_play_xg'] if is_home else match_xg['a_open_play_xg'])
                team_stats[team]["header_xg"].append(match_xg['h_header_xg'] if is_home else match_xg['a_header_xg'])
...
baseline_loss, baseline_acc = train_and_evaluate(feat_df, BASE_FEATURES, "Baseline (V2)")

new_feature_list = ['yellows_diff', 'corners_diff', 'set_piece_xg_diff', 'open_play_xg_diff', 
                    'header_xg_diff', 'goal_diff_momentum']
