...
print("\nTesting C values:")
for C in [0.08, 0.1, 0.12, 0.15, 0.18, 0.2]:
    feat_df = create_features(df, defaults, HOME_ADV=82, K_FACTOR=28, K_ATT=5, K_DEF=90, ELO_DIVISOR=640, K_XG=10)
    ...
    if test_loss < best_loss:
        best_loss = test_loss
        best_params = {'C': C}
        print(f"  C={C}: Loss={test_loss:.4f}, Acc={acc*100:.1f}% *BEST*")
    else:
        print(f"  C={C}: Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
...
for K_XG in [8, 10, 12, 15]:
    feat_df = create_features(df, defaults, HOME_ADV=82, K_FACTOR=28, K_ATT=5, K_DEF=90, ELO_DIVISOR=640, K_XG=K_XG)
    ...
    model = LogisticRegression(penalty='l2', solver='lbfgs', C=best_C, max_iter=2000)
    model.fit(X_train_s, y_train)
    ...
    if test_loss < best_loss:
        best_loss = test_loss
        best_params['K_XG'] = K_XG
        print(f"  K_XG={K_XG}: Loss={test_loss:.4f}, Acc={acc*100:.1f}% *BEST*")
    else:
        print(f"  K_XG={K_XG}: Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
