...
# Check available columns in main data
print("\n1. AVAILABLE COLUMNS IN MAIN DATA:")
print(df.columns.tolist())

# Check stats
print("\n2. STATS SUMMARY:")
print(f"Corners (HC/AC): Home avg={df['HC'].mean():.1f}, Away avg={df['AC'].mean():.1f}")
print(f"Fouls (HF/AF): Home avg={df['HF'].mean():.1f}, Away avg={df['AF'].mean():.1f}")
print(f"Yellow cards (HY/AY): Home avg={df['HY'].mean():.2f}, Away avg={df['AY'].mean():.2f}")
print(f"Red cards (HR/AR): Home avg={df['HR'].mean():.3f}, Away avg={df['AR'].mean():.3f}")
...
# Calculate xGA from shots
print("\n4. CALCULATE XGA (Expected Goals Against):")
shots['xG'] = shots['xG'].astype(float)

# xGA for home team = sum of away team shots xG
# xGA for away team = sum of home team shots xG
match_xga = shots.groupby('match_id').apply(
    lambda x: pd.Series({
        'h_xGA': x[x['h_a'] == 'a']['xG'].sum(),  # Home team concedes away shots
        'a_xGA': x[x['h_a'] == 'h']['xG'].sum(),  # Away team concedes home shots
    })
).reset_index()

print(f"Home xGA avg: {match_xga['h_xGA'].mean():.3f}")
print(f"Away xGA avg: {match_xga['a_xGA'].mean():.3f}")

# Merge with xG data
xg_df = xg_df.merge(match_xga, on='match_id')
print(f"xG data with xGA: {len(xg_df)} matches")
...
def create_features_extended(df, defaults, use_new_features=False):
    elo = {}
    elo_att, elo_def = {}, {}
    elo_xg = {}
    elo_xga = {}  # NEW: xGA-based ELO
    ...
    for _, m in df.iterrows():
        ...
        xga_elo_diff = h_xga_elo - a_xga_elo  # Lower is better (concede less)
        
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "corners": [], "fouls": []})
        a_hist = team_stats.get(away, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "corners": [], "fouls": []})
        ...
        # New features
        h_corners = np.mean(h_hist["corners"][-3:]) if h_hist["corners"] else d['avg_corners']
        a_corners = np.mean(a_hist["corners"][-3:]) if a_hist["corners"] else d['avg_corners']
        h_fouls = np.mean(h_hist["fouls"][-3:]) if h_hist["fouls"] else d['avg_fouls']
        a_fouls = np.mean(a_hist["fouls"][-3:]) if a_hist["fouls"] else d['avg_fouls']
        ...
        if use_new_features:
            feat["xga_elo_diff"] = xga_elo_diff
        ...
        if match_xg:
            ...
            # xGA ELO (lower xGA = better defense)
            h_xga = match_xg['h_xGA']
            a_xga = match_xg['a_xGA']
            # Home team "wins" defensively if they concede less xGA
        ...
        for team, is_home in [(home, True), (away, False)]:
            if team not in team_stats:
                team_stats[team] = {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "corners": [], "fouls": []}
...
FEATURE_COLS_BASE = ["home_elo", "away_elo", "elo_diff", "elo_expected", "att_def_matchup",
                     ...
                     "cs_3_diff", "shots_diff", "xg_elo_diff"]

FEATURE_COLS_NEW = FEATURE_COLS_BASE + ["xga_elo_diff", "corners_diff", "fouls_diff"]
...
feat_df = create_features_extended(df, defaults, use_new_features=False)
train_and_evaluate(feat_df, FEATURE_COLS_BASE, "Current best (with xG ELO)")

feat_df = create_features_extended(df, defaults, use_new_features=True)
train_and_evaluate(feat_df, FEATURE_COLS_NEW, "With xGA ELO + corners + fouls")

# Try individual features
print("\nIndividual new features:")
for new_feat in ["xga_elo_diff", "corners_diff", "fouls_diff"]:
    cols = FEATURE_COLS_BASE + [new_feat]
    train_and_evaluate(feat_df, cols, f"+ {new_feat}")
