...
def create_features(df, home_adv, k_factor, k_att, k_def, defaults, elo_divisor=400):
    ...
    for _, m in df.iterrows():
        ...
        # expected_diff = proxy xG (SOT * conversion rate)
...
print(f"\nParams: HOME_ADV={HOME_ADV}, K={K_FACTOR}, K_ATT={K_ATT}, K_DEF={K_DEF}, C={C}, DIV={ELO_DIVISOR}")
print(f"\nResults:")
...
for season in test_seasons:
    ...
    s_draw = ((s_pred == 'D') & (s_y == 'D')).sum()
    s_draw_pred = (s_pred == 'D').sum()
    print(f"  {season}: Loss={s_loss:.4f}, Acc={s_acc*100:.1f}%, Draw={s_draw}/{s_draw_pred}")

print(f"\nConfusion Matrix:")
cm = confusion_matrix(y_test, y_pred, labels=CLASSES)
print(f"        Pred A  Pred D  Pred H")
for i, c in enumerate(CLASSES):
    print(f"  Act {c}:  {cm[i,0]:5d}   {cm[i,1]:5d}   {cm[i,2]:5d}")

draw_actual = (y_test == 'D').sum()
draw_pred = (y_pred == 'D').sum()
draw_correct = ((y_pred == 'D') & (y_test == 'D')).sum()
print(f"\nDraw: {draw_correct}/{draw_pred} correct, recall={draw_correct/draw_actual*100:.1f}%")
...
with open("epl_model_best.pkl", "wb") as f:
    pickle.dump({
        "model": model,
        "scaler": scaler,
