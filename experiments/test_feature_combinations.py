...
print("TEST FEATURE COMBINATIONS (NO DATA LEAKAGE)")
print("=" * 60)
...
# 1. Baseline (no xG)
evaluate(BASE_FEATURES, "1. Baseline (no xG)")

# 2. + xG ELO only
evaluate(BASE_FEATURES + ["xg_elo_diff"], "2. + xG ELO")

# 3. + xG + xGA ELO
evaluate(BASE_FEATURES + ["xg_elo_diff", "xga_elo_diff"], "3. + xG + xGA ELO")

# 4. + referee_bias (historical)
evaluate(BASE_FEATURES + ["xg_elo_diff", "xga_elo_diff", "referee_bias"], "4. + referee_bias")

# 5. + fouls_diff
evaluate(BASE_FEATURES + ["xg_elo_diff", "xga_elo_diff", "fouls_diff"], "5. + fouls_diff")

# 6. All features
evaluate(BASE_FEATURES + ["xg_elo_diff", "xga_elo_diff", "referee_bias", "fouls_diff"], "6. All features")

print("\n" + "=" * 60)
print("SUMMARY: Each feature's contribution (correct, no leakage)")
