...
# Load improved xG data
xg_df = pd.read_csv("understat_data/our_match_xg_v2.csv")

# Also load original shots to get date/team info
...
print("TRAIN WITH IMPROVED XG (v2 with lastAction)")
print("-" * 60)
...
for _, row in xg_df.iterrows():
    ...
    a_team = row['a_team']
    date = row['date'].date()
    data = {'h_xG': row['h_xG_v2'], 'a_xG': row['a_xG_v2']}
...
# Check match rate
found = sum(1 for _, row in df.iterrows() if get_xg(row['HomeTeam'], row['AwayTeam'], row['Date'].date()))
print(f"xG data found: {found}/{len(df)} matches ({found/len(df)*100:.1f}%)")
...
def create_features(df, defaults, use_xg_rolling=False, replace_expected_diff=False):
    elo, elo_att, elo_def = {}, {}, {}
    ...
    for _, m in df.iterrows():
        ...
        # SOT-based expected diff (proxy xG)
        ...
        # Real xG rolling (from Understat data)
        h_xg_hist = team_xg_history.get(home, [])
        a_xg_hist = team_xg_history.get(away, [])
        h_xg_rolling = np.mean(h_xg_hist[-5:]) if h_xg_hist else d['avg_xg']
        a_xg_rolling = np.mean(a_xg_hist[-5:]) if a_xg_hist else d['avg_xg']
        
        # Choose which expected_diff to use
        if replace_expected_diff and len(h_xg_hist) >= 3 and len(a_xg_hist) >= 3:
            expected_diff = h_xg_rolling - a_xg_rolling
        else:
            expected_diff = home_xg_proxy - away_xg_proxy
        ...
        feat = {
            ...
            "att_def_matchup": att_def, "expected_diff": expected_diff,
        ...
        if use_xg_rolling:
            feat["h_xg_rolling"] = h_xg_rolling
            feat["a_xg_rolling"] = a_xg_rolling
            feat["xg_diff_rolling"] = h_xg_rolling - a_xg_rolling
...
# Baseline
feat_df = create_features(df, defaults, use_xg_rolling=False, replace_expected_diff=False)
train_and_evaluate(feat_df, FEATURE_COLS_BASE, "Baseline (SOT proxy)")

# Replace expected_diff with real xG
feat_df = create_features(df, defaults, use_xg_rolling=False, replace_expected_diff=True)
train_and_evaluate(feat_df, FEATURE_COLS_BASE, "Replace expected_diff with xG")

# Add xG rolling as extra features
FEATURE_COLS_XG = FEATURE_COLS_BASE + ["h_xg_rolling", "a_xg_rolling", "xg_diff_rolling"]
feat_df = create_features(df, defaults, use_xg_rolling=True, replace_expected_diff=False)
train_and_evaluate(feat_df, FEATURE_COLS_XG, "Add xG rolling features")

# Both
feat_df = create_features(df, defaults, use_xg_rolling=True, replace_expected_diff=True)
train_and_evaluate(feat_df, FEATURE_COLS_XG, "Replace + Add xG features")
