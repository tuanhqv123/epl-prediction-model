...
def create_features(df, defaults, new_features=None):
    ...
    for _, m in df.iterrows():
        ...
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "gf": []})
        a_hist = team_stats.get(away, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "gf": []})
        ...
        # Goal difference
        h_gd = sum(h_hist["gf"][-5:]) - sum(h_hist["ga"][-5:]) if h_hist["gf"] else 0
        a_gd = sum(a_hist["gf"][-5:]) - sum(a_hist["ga"][-5:]) if a_hist["gf"] else 0
        ...
        if 'gd_diff' in new_features: feat['gd_diff'] = h_gd - a_gd
        if 'xg_x_xga' in new_features: feat['xg_x_xga'] = xg_elo_diff * xga_elo_diff / 1000
        if 'elo_squared' in new_features: feat['elo_squared'] = elo_diff ** 2 / 10000
        if 'form_consistency' in new_features:
            h_cons = np.std([1 if r == 'W' else 0 for r in h_hist["results"][-5:]]) if len(h_hist["results"]) >= 5 else 0.5
            a_cons = np.std([1 if r == 'W' else 0 for r in a_hist["results"][-5:]]) if len(a_hist["results"]) >= 5 else 0.5
            feat['form_consistency'] = a_cons - h_cons  # Lower std = more consistent = better
        ...
        for team, is_home in [(home, True), (away, False)]:
            if team not in team_stats:
                team_stats[team] = {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "gf": []}
            ...
            team_stats[team]["gf"].append(m["FTHG"] if is_home else m["FTAG"])
...
print("TEST MORE FEATURE COMBINATIONS")
print("=" * 60)
...
baseline_loss, _ = evaluate(feat_df, BASE_FEATURES, "Baseline")

new_features = ['elo_x_form', 'gd_diff', 'xg_x_xga', 'elo_squared', 'form_consistency']
...
if 'elo_x_form' in beneficial:
    feat_df = create_features(df, defaults, new_features=['elo_x_form'])
    evaluate(feat_df, BASE_FEATURES + ['elo_x_form'], "Best: + elo_x_form")

# Try different C values
print("\n" + "-" * 60)
print("Testing different C values with elo_x_form:")
feat_df = create_features(df, defaults, new_features=['elo_x_form'])
cols = BASE_FEATURES + ['elo_x_form']
for c_val in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]:
    ...
    model = LogisticRegression(penalty='l2', solver='lbfgs', C=c_val, max_iter=2000)
    model.fit(X_train_s, y_train)
    ...
    test_loss = log_loss(y_test, proba_ord, labels=CLASSES)
    acc = accuracy_score(y_test, y_pred)
    print(f"  C={c_val}: Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
