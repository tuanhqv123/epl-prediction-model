...
def calculate_defaults(df_subset):
    ...
    defaults['avg_xg'] = 1.35  # Average xG per team per match
    return defaults
...
def create_features(df, home_adv, k_factor, k_att, k_def, defaults, elo_divisor=400):
    ...
    for _, m in df.iterrows():
        ...
        features.append({
            ...
            "xg_diff": h_xg_rolling - a_xg_rolling,
            "h_xg_rolling": h_xg_rolling, "a_xg_rolling": a_xg_rolling,
        })
...
print("MODEL WITH XG FEATURES")
print("-" * 60)

feat_df = create_features(df, HOME_ADV, K_FACTOR, K_ATT, K_DEF, defaults, ELO_DIVISOR)
...
for i, c in enumerate(CLASSES):
    ...
    diff = abs(pred - actual) * 100
    print(f"  {c}: Pred={pred*100:.1f}%, Actual={actual*100:.1f}%, Diff={diff:.1f}%")
...
coef_df = pd.DataFrame({
    ...
    'coef': model.coef_[0]  # Home class
}).sort_values('coef', key=abs, ascending=False)
print(coef_df.head(10).to_string(index=False))

print(f"\nBaseline (without xG): Loss=0.9362, Acc=56.7%")
