...
def create_features(df, defaults, HOME_ADV, K_FACTOR, K_ATT, K_DEF, ELO_DIVISOR, K_XG):
    ...
    for _, m in df.iterrows():
        ...
        h2h_away = sum(1 for x in h2h if x == "A") / len(h2h) if len(h2h) >= 7 else d['home_win_rate']
...
print("HYPERPARAMETER TUNING FOR V2 MODEL")
print("=" * 60)
...
C_values = [0.1, 0.15, 0.2, 0.25]
K_XG_values = [8, 10, 12, 15]
HOME_ADV_values = [75, 82, 90]

total = len(C_values) * len(K_XG_values) * len(HOME_ADV_values)
count = 0

for C, K_XG, HOME_ADV in product(C_values, K_XG_values, HOME_ADV_values):
    count += 1
    feat_df = create_features(df, defaults, HOME_ADV=HOME_ADV, K_FACTOR=28, K_ATT=5, K_DEF=90, ELO_DIVISOR=640, K_XG=K_XG)
    ...
    if test_loss < best_loss:
        best_loss = test_loss
        best_params = {'C': C, 'K_XG': K_XG, 'HOME_ADV': HOME_ADV}
        print(f"[{count}/{total}] NEW BEST: C={C}, K_XG={K_XG}, HOME_ADV={HOME_ADV} -> Loss={test_loss:.4f}, Acc={acc*100:.1f}%")

print(f"\nBEST PARAMS: {best_params}")
print(f"BEST LOSS: {best_loss:.4f}")
print(f"\nComparison:")
print(f"  Original V2: Loss=0.9292")
print(f"  Tuned V2:    Loss={best_loss:.4f}")
print(f"  Improvement: {(0.9292 - best_loss):.4f}")
