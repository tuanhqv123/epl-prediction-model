"""
Test rolling xG features and big chances
"""
import sys
sys.path.insert(0, '.')
import numpy as np
...
# Load shots and calculate per-match xG stats
shots = pd.read_csv("understat_data/epl_shots_all.csv")
shots['xG'] = shots['xG'].astype(float)

match_xg_stats = shots.groupby('match_id').apply(
    lambda x: pd.Series({
...
match_xg_stats = match_xg_stats.merge(match_info, on='match_id')
match_xg_stats['date'] = pd.to_datetime(match_xg_stats['date'])

print(f"Matches with xG stats: {len(match_xg_stats)}")

# Build team xG history lookup
team_mapping = {
...
# Build chronological xG history per team
team_xg_history = {}
for _, row in match_xg_stats.sort_values('date').iterrows():
    h_team, a_team, date = row['h_team'], row['a_team'], row['date']
...
# Calculate xG defaults from training data
train_xg = match_xg_stats[match_xg_stats['season'] < 2024]
defaults['avg_xG'] = (train_xg['h_xG'].mean() + train_xg['a_xG'].mean()) / 2
defaults['avg_big_chances'] = (train_xg['h_big_chances'].mean() + train_xg['a_big_chances'].mean()) / 2

print(f"avg_xG: {defaults['avg_xG']:.3f}")
print(f"avg_big_chances: {defaults['avg_big_chances']:.3f}")
...
def get_team_rolling_xg(team, match_date, n=5):
    """Get rolling xG stats for team BEFORE match_date"""
    team_mapped = reverse_mapping.get(team, team)
    if team_mapped not in team_xg_history:
        team_mapped = team
    if team_mapped not in team_xg_history:
        return None
    
    history = team_xg_history[team_mapped]
    recent = [h for h in history if h['date'] < match_date][-n:]
    ...
    return {
        ...
        'xGA': np.mean([h['xGA'] for h in recent]),
        'big_chances': np.mean([h['big_chances'] for h in recent]),
        'xG_diff': np.mean([h['xG'] - h['xGA'] for h in recent]),
    }

def create_features(df, defaults, extra=[]):
    ...
    for _, m in df.iterrows():
        ...
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "home_results": [], "away_results": []})
        a_hist = team_stats.get(away, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "home_results": [], "away_results": []})
        ...
        h_home_wins = sum(1 for r in h_hist["home_results"][-5:] if r == 'W') / max(len(h_hist["home_results"][-5:]), 1) if h_hist["home_results"] else d['home_win_rate']
        a_away_wins = sum(1 for r in a_hist["away_results"][-5:] if r == 'W') / max(len(a_hist["away_results"][-5:]), 1) if a_hist["away_results"] else d['away_win_rate']
        ...
        feat = {
            ...
            "home_away_form": h_home_wins - a_away_wins,
        }
        
        # NEW: Rolling xG features
        if "rolling_xg_diff" in extra or "big_chances_diff" in extra or "xg_form_diff" in extra:
            h_rolling = get_team_rolling_xg(home, match_date, n=5)
            a_rolling = get_team_rolling_xg(away, match_date, n=5)
            
            if "rolling_xg_diff" in extra:
                h_xg_roll = h_rolling['xG'] if h_rolling else d['avg_xG']
                a_xg_roll = a_rolling['xG'] if a_rolling else d['avg_xG']
                feat["rolling_xg_diff"] = h_xg_roll - a_xg_roll
            
            if "big_chances_diff" in extra:
                h_bc = h_rolling['big_chances'] if h_rolling else d['avg_big_chances']
                a_bc = a_rolling['big_chances'] if a_rolling else d['avg_big_chances']
                feat["big_chances_diff"] = h_bc - a_bc
            
            if "xg_form_diff" in extra:
                h_xg_form = h_rolling['xG_diff'] if h_rolling else 0
                a_xg_form = a_rolling['xG_diff'] if a_rolling else 0
                feat["xg_form_diff"] = h_xg_form - a_xg_form
        ...
        for team, is_home in [(home, True), (away, False)]:
            if team not in team_stats:
                team_stats[team] = {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "home_results": [], "away_results": []}
...
def test_model(extra, name):
    feat_df = create_features(df, defaults, extra)
    cols = BASE_FEATURES + extra
    
    train_mask = df["season"].isin(train_seasons).values
    test_mask = df["season"].isin(test_seasons).values
    
    X_train = feat_df[train_mask][cols].fillna(0).replace([np.inf, -np.inf], 0).values
    y_train = feat_df[train_mask]["FTR"].values
    X_test = feat_df[test_mask][cols].fillna(0).replace([np.inf, -np.inf], 0).values
...
print("\n" + "="*60)
print("TESTING ROLLING XG FEATURES")
print("="*60)

baseline = test_model([], "Baseline (current best)")
test_model(["rolling_xg_diff"], "+ rolling_xg_diff")
test_model(["big_chances_diff"], "+ big_chances_diff")
test_model(["xg_form_diff"], "+ xg_form_diff")
test_model(["rolling_xg_diff", "big_chances_diff"], "+ rolling_xg + big_chances")
test_model(["rolling_xg_diff", "xg_form_diff"], "+ rolling_xg + xg_form")
test_model(["rolling_xg_diff", "big_chances_diff", "xg_form_diff"], "All xG features")


print("\n" + "="*60)
print("FINE-TUNING xg_form_diff")
print("="*60)

# Test different window sizes
def create_features_v2(df, defaults, window=5):
    elo, elo_att, elo_def, elo_xg, elo_xga = {}, {}, {}, {}, {}
    team_stats, h2h_history, team_last_epl = {}, {}, {}
    referee_history = {}
    features = []
    ...
    def get_team_rolling_xg_v2(team, match_date, n):
        team_mapped = reverse_mapping.get(team, team)
        if team_mapped not in team_xg_history:
            team_mapped = team
        if team_mapped not in team_xg_history:
    ...
    for _, m in df.iterrows():
        ...
        # xG form diff with variable window
        h_rolling = get_team_rolling_xg_v2(home, match_date, n=window)
        a_rolling = get_team_rolling_xg_v2(away, match_date, n=window)
        h_xg_form = h_rolling['xG_diff'] if h_rolling else 0
        a_xg_form = a_rolling['xG_diff'] if a_rolling else 0
        ...
        feat = {
            ...
            "elo_x_form": elo_diff * (h_winrate - a_winrate),
            "home_away_form": h_home_wins - a_away_wins,
            "xg_form_diff": h_xg_form - a_xg_form,
        }
        features.append(feat)
...
COLS = BASE_FEATURES + ["xg_form_diff"]
...
print("\nTesting larger windows:")
for window in [12, 15, 20]:
    feat_df = create_features_v2(df, defaults, window)
