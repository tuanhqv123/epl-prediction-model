...
print("FINAL MODEL V3 - BEST MODEL")
print("=" * 60)
print("Features: Base + xG ELO + xGA ELO + referee_bias + fouls_diff")
...
print(f"2. + xG ELO:              Loss=0.9333, Acc=56.8%  (+0.0029)")
print(f"3. + xGA ELO:             Loss=0.9331, Acc=57.1%  (+0.0002)")
print(f"4. + referee_bias:        Loss=0.9295, Acc=57.8%  (+0.0036)")
print(f"5. + fouls_diff:          Loss={test_loss:.4f}, Acc={acc*100:.1f}%  (+0.0003)")
...
print(f"\nTOP 10 FEATURE IMPORTANCE (for Home win):")
print("-" * 60)
coef_df = pd.DataFrame({'feature': FEATURE_COLS, 'coef_H': model.coef_[2]}).sort_values('coef_H', key=abs, ascending=False)
print(coef_df.head(10).to_string(index=False))

params = {
    'home_adv': HOME_ADV, 'k_factor': K_FACTOR, 'k_att': K_ATT, 'k_def': K_DEF,
    'k_xg': K_XG, 'C': C, 'elo_divisor': ELO_DIVISOR
}

with open("epl_model_final.pkl", "wb") as f:
    pickle.dump({
        "model": model, "scaler": scaler, "hyperparameters": params,
        "defaults": defaults, "feature_cols": FEATURE_COLS, "classes": CLASSES,
        "test_loss": test_loss, "referee_stats": referee_stats
    }, f)
