...
# Fine-tune K_XG
print("Fine-tune K_XG:")
for k_xg in [6, 7, 8, 9, 10, 11, 12, 13, 14]:
...
# Best model details
print("\n" + "=" * 60)
print("BEST MODEL (K_XG=10)")
print("=" * 60)

feat_df = create_features_with_xg_elo(df, defaults, 10)
...
print(f"\nFeature Importance (xg_elo_diff):")
coef_df = pd.DataFrame({
    'feature': FEATURE_COLS,
    'coef_H': model.coef_[2]
}).sort_values('coef_H', key=abs, ascending=False)
print(coef_df.head(10).to_string(index=False))

print(f"\nComparison:")
print(f"  Baseline: Loss=0.9362, Acc=56.7%")
print(f"  With xG ELO: Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
