...
print("FIX REFEREE BIAS - USE HISTORICAL DATA ONLY")
print("=" * 60)
...
def create_features_fixed(df, defaults):
    elo, elo_att, elo_def, elo_xg, elo_xga = {}, {}, {}, {}, {}
    team_stats, h2h_history, team_last_epl = {}, {}, {}
    referee_history = {}  # Track referee stats historically
    features = []
    d = defaults
    ...
    for _, m in df.iterrows():
        ...
        # FIXED: Calculate referee home rate from HISTORICAL data only
        ref_stats = referee_history.get(ref, {'home_wins': 0, 'total': 0})
        if ref_stats['total'] >= 10:
            ref_home_rate = ref_stats['home_wins'] / ref_stats['total']
        else:
            ref_home_rate = d['home_win_rate']  # Use default if not enough data
        ...
        # Update ELOs AFTER creating features
        ...
        # Update referee history AFTER creating features
...
# Test with fixed referee bias
print("\nTesting with FIXED referee_bias (historical only):")
print("-" * 60)

feat_df = create_features_fixed(df, defaults)
...
print(f"With FIXED referee_bias: Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
print(f"\nComparison:")
print(f"  Old (data leakage):  Loss=0.9292, Acc=57.8%")
print(f"  Fixed (no leakage):  Loss={test_loss:.4f}, Acc={acc*100:.1f}%")

# Also test without referee_bias at all
print("\n" + "-" * 60)
print("Testing WITHOUT referee_bias:")

FEATURE_COLS_NO_REF = [c for c in FEATURE_COLS if c != 'referee_bias']
X_train2 = feat_df[train_mask][FEATURE_COLS_NO_REF].fillna(0).replace([np.inf, -np.inf], 0).values
X_test2 = feat_df[test_mask][FEATURE_COLS_NO_REF].fillna(0).replace([np.inf, -np.inf], 0).values

scaler2 = StandardScaler()
X_train_s2 = scaler2.fit_transform(X_train2)
X_test_s2 = scaler2.transform(X_test2)

model2 = LogisticRegression(penalty='l2', solver='lbfgs', C=C, max_iter=2000)
model2.fit(X_train_s2, y_train)

proba2 = model2.predict_proba(X_test_s2)
proba_ord2 = np.column_stack([proba2[:, list(model2.classes_).index(c)] for c in CLASSES])
y_pred2 = model2.predict(X_test_s2)

test_loss2 = log_loss(y_test, proba_ord2, labels=CLASSES)
acc2 = accuracy_score(y_test, y_pred2)

print(f"Without referee_bias: Loss={test_loss2:.4f}, Acc={acc2*100:.1f}%")

print("\n" + "=" * 60)
print("CONCLUSION:")
if test_loss < test_loss2:
    print(f"Fixed referee_bias HELPS: {test_loss2 - test_loss:.4f} improvement")
else:
    print(f"Fixed referee_bias HURTS: {test_loss - test_loss2:.4f} worse")
    print("Should REMOVE referee_bias from model")
