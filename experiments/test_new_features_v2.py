"""
Test new features on top of current best model
"""
import sys
sys.path.insert(0, '.')
import numpy as np
...
def create_features(df, defaults, extra_features=[]):
    ...
    for _, m in df.iterrows():
        ...
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "corners": [], "yellows": [], "goals": []})
        a_hist = team_stats.get(away, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "corners": [], "yellows": [], "goals": []})
        ...
        # New features
        if "corners_diff" in extra_features:
        ...
        if "yellows_diff" in extra_features:
        ...
        if "shot_accuracy_diff" in extra_features:
            h_shots_l = h_hist["shots"][-5:] if h_hist["shots"] else [d['avg_home_shots']]
            h_sot_l = h_hist["sot"][-5:] if h_hist["sot"] else [d['avg_home_shots']*0.35]
            a_shots_l = a_hist["shots"][-5:] if a_hist["shots"] else [d['avg_away_shots']]
            a_sot_l = a_hist["sot"][-5:] if a_hist["sot"] else [d['avg_away_shots']*0.35]
            h_acc = sum(h_sot_l) / (sum(h_shots_l) + 0.1)
            a_acc = sum(a_sot_l) / (sum(a_shots_l) + 0.1)
            feat["shot_accuracy_diff"] = h_acc - a_acc
        
        if "goal_efficiency" in extra_features:
            h_goals_l = h_hist["goals"][-5:] if h_hist["goals"] else [d['avg_home_goals']]
            h_sot_l = h_hist["sot"][-5:] if h_hist["sot"] else [d['avg_home_shots']*0.35]
            a_goals_l = a_hist["goals"][-5:] if a_hist["goals"] else [d['avg_away_goals']]
            a_sot_l = a_hist["sot"][-5:] if a_hist["sot"] else [d['avg_away_shots']*0.35]
            h_eff = sum(h_goals_l) / (sum(h_sot_l) + 0.1)
            a_eff = sum(a_goals_l) / (sum(a_sot_l) + 0.1)
            feat["goal_efficiency"] = h_eff - a_eff
        
        if "momentum" in extra_features:
            h_recent = h_hist["results"][-3:]
            a_recent = a_hist["results"][-3:]
            h_mom = sum(1 for r in h_recent if r == 'W') - sum(1 for r in h_recent if r == 'L')
            a_mom = sum(1 for r in a_recent if r == 'W') - sum(1 for r in a_recent if r == 'L')
            feat["momentum"] = h_mom - a_mom
        
        if "goals_scored_diff" in extra_features:
            ...
            feat["goals_scored_diff"] = h_goals_avg - a_goals_avg
        
        if "elo_x_corners" in extra_features:
            h_corners = np.mean(h_hist["corners"][-5:]) if h_hist["corners"] else d['avg_corners']
            a_corners = np.mean(a_hist["corners"][-5:]) if a_hist["corners"] else d['avg_corners']
            feat["elo_x_corners"] = elo_diff * (h_corners - a_corners) / 100
        ...
        for team, is_home in [(home, True), (away, False)]:
            if team not in team_stats:
                team_stats[team] = {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], "fouls": [], "corners": [], "yellows": [], "goals": []}
...
def test_model(extra_features, name):
    feat_df = create_features(df, defaults, extra_features)
...
print("="*60)
print("TESTING NEW FEATURES ON TOP OF CURRENT BEST MODEL")
print("="*60)

baseline_loss, baseline_acc = test_model([], "Baseline (current best)")

print("\nSingle features:")
test_model(["corners_diff"], "+ corners_diff")
test_model(["yellows_diff"], "+ yellows_diff")
test_model(["shot_accuracy_diff"], "+ shot_accuracy_diff")
test_model(["goal_efficiency"], "+ goal_efficiency")
test_model(["momentum"], "+ momentum")
test_model(["goals_scored_diff"], "+ goals_scored_diff")
test_model(["elo_x_corners"], "+ elo_x_corners")

print("\nCombinations:")
test_model(["momentum", "goal_efficiency"], "+ momentum + goal_eff")
test_model(["shot_accuracy_diff", "goal_efficiency"], "+ shot_acc + goal_eff")
test_model(["corners_diff", "momentum"], "+ corners + momentum")
test_model(["momentum", "goals_scored_diff"], "+ momentum + goals_scored")
