"""
FINAL MODEL - STRICTLY CORRECT (NO DATA LEAKAGE AT ALL)
=======================================================
This version fixes the xG model leak by training on train data only.
"""
...
print("FINAL MODEL - STRICTLY CORRECT")
print("=" * 70)
...
# ============================================================
# STEP 1: BUILD XG MODEL FROM TRAIN DATA ONLY
# ============================================================
print("\n1. BUILDING XG MODEL (train data only):")
print("-" * 70)
...
shots['lastAction'] = shots['lastAction'].fillna('Unknown')

shot_type_dummies = pd.get_dummies(shots['shotType'], prefix='shot')
situation_dummies = pd.get_dummies(shots['situation'], prefix='sit')
lastaction_dummies = pd.get_dummies(shots['lastAction'], prefix='action')

numeric_features = ['X', 'Y', 'distance_to_goal', 'angle', 'central_position', 
                    'distance_sq', 'inside_box', 'very_close', 'is_home']

X_shots = pd.concat([shots[numeric_features], shot_type_dummies, situation_dummies, lastaction_dummies], axis=1)
y_shots = shots['is_goal']

# Train xG model on TRAIN seasons only
xg_train_mask = shots['season'] < 2024  # 2014-2023
X_xg_train = X_shots[xg_train_mask]
y_xg_train = y_shots[xg_train_mask]

xg_model = LogisticRegression(max_iter=1000, C=1.0)
xg_model.fit(X_xg_train, y_xg_train)

print(f"xG model trained on {len(X_xg_train)} shots (seasons < 2024)")
print(f"xG model AUC on train: {roc_auc_score(y_xg_train, xg_model.predict_proba(X_xg_train)[:, 1]):.4f}")

# Predict xG for ALL shots (but model only saw train data)
shots['our_xG'] = xg_model.predict_proba(X_shots)[:, 1]

# Calculate match xG
match_xg = shots.groupby('match_id').apply(
    lambda x: pd.Series({
        'h_xG': x[x['h_a'] == 'h']['our_xG'].sum(),
        'a_xG': x[x['h_a'] == 'a']['our_xG'].sum(),
        'h_xGA': x[x['h_a'] == 'a']['our_xG'].sum(),  # xGA = opponent's xG
        'a_xGA': x[x['h_a'] == 'h']['our_xG'].sum(),
    }), include_groups=False
).reset_index()

match_info = shots.groupby('match_id').first()[['date', 'h_team', 'a_team', 'season']].reset_index()
xg_df = match_xg.merge(match_info, on='match_id')
xg_df['date'] = pd.to_datetime(xg_df['date'])

print(f"Match xG calculated for {len(xg_df)} matches")
...
for _, row in xg_df.iterrows():
    ...
    data = {'h_xG': row['h_xG'], 'a_xG': row['a_xG'], 'h_xGA': row['h_xGA'], 'a_xGA': row['a_xGA']}
...
print(f"\n3. RESULTS (STRICTLY CORRECT - NO LEAKAGE):")
print("-" * 70)
...
with open("epl_model_final.pkl", "wb") as f:
    pickle.dump({
        ...
        "test_loss": test_loss,
        "accuracy": acc,
        "note": "xG model trained on train data only (no leakage)"
    }, f)

print(f"\n4. SAVED: epl_model_final.pkl")
print("   - Includes xG model (trained on train data only)")
print("   - All defaults from training data")
