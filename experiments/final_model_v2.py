...
print("FINAL MODEL V2 (with referee_bias + fouls_diff)")
print("=" * 60)
...
for ref in referee_stats:
    if referee_stats[ref]['total'] >= 10:
        ...
        referee_stats[ref]['home_rate'] = 0.446
...
print(f"\nIMPROVEMENT SUMMARY:")
print("-" * 60)
print(f"Original baseline:     Loss=0.9362, Acc=56.7%")
print(f"With xG+xGA ELO:       Loss=0.9331, Acc=57.1%")
print(f"With referee+fouls:    Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
print(f"Total improvement:     {(0.9362 - test_loss):.4f} ({(0.9362 - test_loss)/0.9362*100:.2f}%)")
...
print(f"\nFEATURE IMPORTANCE (for Home win):")
print("-" * 60)
...
with open("epl_model_v2.pkl", "wb") as f:
    pickle.dump({
