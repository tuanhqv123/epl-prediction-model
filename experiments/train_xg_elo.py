...
def create_features_xg_elo(df, home_adv, k_factor, k_att, k_def, defaults, elo_divisor, use_xg_for_att_def=True):
    elo, elo_att, elo_def = {}, {}, {}
    ...
    for _, m in df.iterrows():
        ...
        # Use xG instead of goals for att/def ELO if available
        h_goals, a_goals = m["FTHG"], m["FTAG"]
        xg_key = (home, away, match_date.date())
        
        if use_xg_for_att_def and xg_key in xg_lookup:
            match_xg = xg_lookup[xg_key]
            h_xg_val = match_xg['h_xg']
            a_xg_val = match_xg['a_xg']
            elo_att[home] = h_att + k_att * (h_xg_val - d['avg_xg']) / d['xg_std']
            elo_def[home] = h_def + k_def * (d['avg_xg'] - a_xg_val) / d['xg_std']
            elo_att[away] = a_att + k_att * (a_xg_val - d['avg_xg']) / d['xg_std']
            elo_def[away] = a_def + k_def * (d['avg_xg'] - h_xg_val) / d['xg_std']
        else:
...
print("COMPARE: GOALS-BASED ELO vs XG-BASED ELO")
print("-" * 60)

for use_xg in [False, True]:
    label = "xG-based" if use_xg else "Goals-based"
    feat_df = create_features_xg_elo(df, HOME_ADV, K_FACTOR, K_ATT, K_DEF, defaults, ELO_DIVISOR, use_xg_for_att_def=use_xg)
