...
# Load our xG data (calculated from raw shot data)
...
print("INTEGRATE OUR XG INTO MODEL")
print("-" * 60)

# Build xG lookup with team name mapping
team_mapping = {
...
# Check match rate
found = sum(1 for _, row in df.iterrows() if get_match_xg(row['HomeTeam'], row['AwayTeam'], row['Date'].date()))
...
def create_features(df, home_adv, k_factor, k_att, k_def, defaults, elo_divisor, use_xg=False):
    elo, elo_att, elo_def = {}, {}, {}
    ...
    for _, m in df.iterrows():
        ...
        if use_xg:
            match_xg = get_match_xg(home, away, match_date.date())
            if match_xg:
...
# Compare baseline vs with xG
print("\n" + "=" * 60)
print("COMPARISON")
print("=" * 60)

feat_df_base = create_features(df, HOME_ADV, K_FACTOR, K_ATT, K_DEF, defaults, ELO_DIVISOR, use_xg=False)
model_base, loss_base, acc_base = train_and_evaluate(feat_df_base, FEATURE_COLS_BASE, "BASELINE (no xG)")

feat_df_xg = create_features(df, HOME_ADV, K_FACTOR, K_ATT, K_DEF, defaults, ELO_DIVISOR, use_xg=True)
model_xg, loss_xg, acc_xg = train_and_evaluate(feat_df_xg, FEATURE_COLS_XG, "WITH OUR XG")

print(f"\n" + "=" * 60)
print(f"SUMMARY")
print("=" * 60)
print(f"Baseline: Loss={loss_base:.4f}, Acc={acc_base*100:.1f}%")
print(f"With xG:  Loss={loss_xg:.4f}, Acc={acc_xg*100:.1f}%")
print(f"Improvement: Loss {(loss_base - loss_xg):.4f}, Acc {(acc_xg - acc_base)*100:.1f}%")
