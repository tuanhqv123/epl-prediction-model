"""
FINAL MODEL V6 - Best combination
Loss: 0.9270, Acc: 58.2%
Only added xg_form_diff (no home_away_form)
"""
import sys
sys.path.insert(0, '.')
import numpy as np
...
# V6: V3 features + xg_form_diff (no home_away_form)
FEATURE_COLS = ["home_elo", "away_elo", "elo_diff", "elo_expected", "att_def_matchup",
                ...
                "xg_form_diff"]  # Only xg_form_diff, no home_away_form
...
print("FINAL MODEL V6 - Best combination")
print("=" * 60)
...
print(f"  Baseline (V1): Loss=0.9362, Acc=56.7%")
print(f"  Current (V6):  Loss={test_loss:.4f}, Acc={acc*100:.1f}%")
print(f"  Loss improvement: {0.9362 - test_loss:.4f} ({(0.9362 - test_loss)/0.9362*100:.2f}%)")
print(f"  Acc improvement:  {acc*100 - 56.7:.1f}%")
...
params = {'home_adv': HOME_ADV, 'k_factor': K_FACTOR, 'k_att': K_ATT, 'k_def': K_DEF,
          'k_xg': K_XG, 'C': C, 'elo_divisor': ELO_DIVISOR, 'xg_form_window': 11}
