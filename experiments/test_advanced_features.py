...
# Calculate shot conversion per match
print("Calculating shot conversion rates...")
shot_conv = shots.groupby(['match_id', 'h_a']).agg({
    'is_goal': 'sum',
    'xG': ['sum', 'count']
}).reset_index()
shot_conv.columns = ['match_id', 'h_a', 'goals', 'xG_sum', 'shots']
shot_conv['conversion'] = shot_conv['goals'] / shot_conv['shots']
shot_conv['xg_overperform'] = shot_conv['goals'] - shot_conv['xG_sum']

h_conv = shot_conv[shot_conv['h_a'] == 'h'][['match_id', 'conversion', 'xg_overperform']].rename(
    columns={'conversion': 'h_conversion', 'xg_overperform': 'h_xg_overperform'})
a_conv = shot_conv[shot_conv['h_a'] == 'a'][['match_id', 'conversion', 'xg_overperform']].rename(
    columns={'conversion': 'a_conversion', 'xg_overperform': 'a_xg_overperform'})
xg_df = xg_df.merge(h_conv, on='match_id', how='left')
xg_df = xg_df.merge(a_conv, on='match_id', how='left')
...
for _, row in xg_df.iterrows():
    ...
    data = {
        'h_xG': row['h_xG_v2'], 'a_xG': row['a_xG_v2'], 
        'h_xGA': row['h_xGA'], 'a_xGA': row['a_xGA'],
        'h_conversion': row.get('h_conversion', 0.11),
        'a_conversion': row.get('a_conversion', 0.11),
        'h_xg_overperform': row.get('h_xg_overperform', 0),
        'a_xg_overperform': row.get('a_xg_overperform', 0),
    }
...
def create_features(df, defaults, new_features=None):
    if new_features is None: new_features = []
    ...
    for _, m in df.iterrows():
        ...
        h_hist = team_stats.get(home, {"sot": [], "draw": [], "results": [], "ga": [], "shots": [], 
                                        "fouls": [], "corners": [], "ht_goals": [], "conversion": [],
                                        "xg_overperform": [], "goals": []})
        ...
        h_conv = np.mean(h_hist["conversion"][-5:]) if h_hist["conversion"] else d['avg_conversion']
        a_conv = np.mean(a_hist["conversion"][-5:]) if a_hist["conversion"] else d['avg_conversion']
        h_xg_over = np.mean(h_hist["xg_overperform"][-5:]) if h_hist["xg_overperform"] else 0
        a_xg_over = np.mean(a_hist["xg_overperform"][-5:]) if a_hist["xg_overperform"] else 0
        ...
        h_momentum = sum([(1 if r == 'W' else -1) * (0.5 ** i) for i, r in enumerate(reversed(h_hist["results"][-5:]))]) if h_hist["results"] else 0
        a_momentum = sum([(1 if r == 'W' else -1) * (0.5 ** i) for i, r in enumerate(reversed(a_hist["results"][-5:]))]) if a_hist["results"] else 0
        ...
        if 'corners_diff' in new_features: feat['corners_diff'] = h_corners - a_corners
        if 'ht_goals_diff' in new_features: feat['ht_goals_diff'] = h_ht_goals - a_ht_goals
        if 'conversion_diff' in new_features: feat['conversion_diff'] = h_conv - a_conv
        if 'xg_overperform_diff' in new_features: feat['xg_overperform_diff'] = h_xg_over - a_xg_over
        if 'goals_avg_diff' in new_features: feat['goals_avg_diff'] = h_goals_avg - a_goals_avg
        if 'momentum_diff' in new_features: feat['momentum_diff'] = h_momentum - a_momentum
        if 'elo_x_form' in new_features: feat['elo_x_form'] = elo_diff * (h_winrate - a_winrate)
        if 'attack_strength' in new_features: feat['attack_strength'] = (h_att - 1500) - (a_def - 1500)
        
        features.append(feat)

        # Update all stats AFTER creating features
        ...
        for team, is_home in [(home, True), (away, False)]:
            ...
            if match_xg:
                team_stats[team]["conversion"].append(match_xg['h_conversion'] if is_home else match_xg['a_conversion'])
                team_stats[team]["xg_overperform"].append(match_xg['h_xg_overperform'] if is_home else match_xg['a_xg_overperform'])
...
def evaluate(feat_df, feature_cols, label):
...
# Baseline
feat_df = create_features(df, defaults, new_features=[])
baseline_loss, baseline_acc = evaluate(feat_df, BASE_FEATURES, "Baseline")

# Test each new feature
new_feature_list = ['corners_diff', 'ht_goals_diff', 'conversion_diff', 'xg_overperform_diff', 
                    'goals_avg_diff', 'momentum_diff', 'elo_x_form', 'attack_strength']

print("\nTesting individual features:")
print("-" * 60)
results = []
for feat in new_feature_list:
    feat_df = create_features(df, defaults, new_features=[feat])
    cols = BASE_FEATURES + [feat]
    loss, acc = evaluate(feat_df, cols, f"+ {feat}")
...
# Test beneficial features together
beneficial = results_df[results_df['improvement'] > 0]['feature'].tolist()
print(f"\nBeneficial features: {beneficial}")
if beneficial:
    feat_df = create_features(df, defaults, new_features=beneficial)
    cols = BASE_FEATURES + beneficial
    evaluate(feat_df, cols, "All beneficial")
