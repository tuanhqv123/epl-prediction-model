"""
Test ALL new features on top of current best model
"""
import sys
sys.path.insert(0, '.')
import numpy as np
...
def create_features(df, defaults, extra=[]):
    ...
    for _, m in df.iterrows():
        ...
        if "ppg_diff" in extra:
            ...
            feat["ppg_diff"] = h_ppg - a_ppg
        
        if "gd_diff" in extra:
            feat["gd_diff"] = (h_table['gd'] - a_table['gd']) / 10
        
        if "home_away_form" in extra:
            ...
            feat["home_away_form"] = h_home_wins - a_away_wins
        
        if "streak_diff" in extra:
            def get_streak(results):
                ...
                return streak
            feat["streak_diff"] = get_streak(h_hist["results"]) - get_streak(a_hist["results"])
...
print("="*60)
print("TESTING NEW FEATURES ON CURRENT BEST MODEL")
print("="*60)

baseline = test_model([], "Baseline")
test_model(["ppg_diff"], "+ ppg_diff")
test_model(["gd_diff"], "+ gd_diff")
test_model(["home_away_form"], "+ home_away_form")
test_model(["streak_diff"], "+ streak_diff")
test_model(["ppg_diff", "gd_diff"], "+ ppg + gd")
test_model(["ppg_diff", "home_away_form"], "+ ppg + home_away")
test_model(["ppg_diff", "gd_diff", "home_away_form", "streak_diff"], "All new")


print("\n" + "="*60)
print("FINE-TUNING home_away_form")
print("="*60)

# Test different window sizes for home_away_form
def create_features_v2(df, defaults, window=5):
    ...
    for _, m in df.iterrows():
        ...
        # Home/away form with variable window
        h_home_wins = sum(1 for r in h_hist["home_results"][-window:] if r == 'W') / max(len(h_hist["home_results"][-window:]), 1) if h_hist["home_results"] else d['home_win_rate']
        a_away_wins = sum(1 for r in a_hist["away_results"][-window:] if r == 'W') / max(len(a_hist["away_results"][-window:]), 1) if a_hist["away_results"] else d['away_win_rate']
...
COLS = BASE_FEATURES + ["home_away_form"]
