"""
FINAL MODEL V5 - Added xg_form_diff feature (window=11)
Loss: 0.9261 (improved from 0.9307)
"""
import sys
sys.path.insert(0, '.')
import numpy as np
...
# Load shots for xG form calculation
...
for _, row in match_xg_stats.sort_values('date').iterrows():
    ...
    for team, is_home in [(h_team, True), (a_team, False)]:
        ...
        if is_home:
            team_xg_history[team].append({'date': date, 'xG': row['h_xG'], 'xGA': row['a_xG']})
        else:
            team_xg_history[team].append({'date': date, 'xG': row['a_xG'], 'xGA': row['h_xG']})

def get_team_rolling_xg(team, match_date, n=11):
    ...
    if not recent:
        return None
    return np.mean([h['xG'] - h['xGA'] for h in recent])

# Load xG data for ELO
...
FEATURE_COLS = ["home_elo", "away_elo", "elo_diff", "elo_expected", "att_def_matchup",
                ...
                "home_away_form", "xg_form_diff"]  # NEW FEATURE
...
def create_features(df, defaults):
    ...
    for _, m in df.iterrows():
        ...
        # xG form diff (rolling xG - xGA over last 11 matches)
        h_xg_form = get_team_rolling_xg(home, match_date, n=11)
        a_xg_form = get_team_rolling_xg(away, match_date, n=11)
        h_xg_form = h_xg_form if h_xg_form is not None else 0
        a_xg_form = a_xg_form if a_xg_form is not None else 0
...
print("FINAL MODEL V5 - with xg_form_diff (window=11)")
print("=" * 60)
...
print(f"  Baseline (V1): 0.9362")
print(f"  Current (V5):  {test_loss:.4f}")
print(f"  Improvement:   {0.9362 - test_loss:.4f} ({(0.9362 - test_loss)/0.9362*100:.2f}%)")
...
with open("epl_model_final.pkl", "wb") as f:
    pickle.dump({"model": model, "scaler": scaler, "hyperparameters": params,
                 ...
                 "test_loss": test_loss, "accuracy": acc}, f)
